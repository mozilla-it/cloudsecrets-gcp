...
[![Build Status](https://travis-ci.com/mozilla-it/cloudsecrets-gcp.svg?branch=master)](https://travis-ci.com/mozilla-it/cloudsecrets-gcp)

cloudsecrets-gcp is a thin layer for cloud vendor secrets managers.

The cloudsecrets-gcp library includes a cli cloud-secrets which exposes all the features of the library.

Current supported platforms include GCP (secretmanager) and nothing else. Use mozilla-it/cloudsecrets for AWS.
...
```
docker build -t mozilla/cloudsecrets-gcp .
```
...
1. git clone {git@github.com:mozilla-it/cloudsecrets-gcp.git | https://github.com/mozilla-it/cloudsecrets-gcp.git}
2. cd cloudsecrets-gcp
...
```
pip3 install git+https://github.com/mozilla-it/cloudsecrets-gcp.git
```
From Pipenv:
```
[packages]
cloudsecrets-gcp = { version = "*", git = "https://github.com/mozilla-it/cloudsecrets-gcp.git", ref = "master", editable = false }
```
